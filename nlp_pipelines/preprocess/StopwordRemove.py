...
class StopwordRemove(BasePreprocessor):
    def __init__(self, model_name="en_core_web_sm", pos_tags_to_remove=[]):
        ...
        self.method_name = "Stopword Removal"
    ...
    def transform(self, dataset: Dataset) -> Dataset:
        ...
        for text in dataset.texts:
            doc = self.nlp(text)
            cleaned_text = " ".join([token.text for token in doc if not token.is_stop and not token.is_space and not token.is_punct])
